...
struct CoversCache<T: Hashable>: CachedComputation {
    static func compute(_ board: SudokuBoard<T>) -> [Position: CoveredValue<T>] {
        var result: [Position: CoveredValue<T>] = [:]
        var cache = Cache(board.slicedGrid)
...
extension Cache {
    ...
    mutating func covers<Value>() -> [Position: CoveredValue<Value>] where Subject == SudokuBoard<Value> {
